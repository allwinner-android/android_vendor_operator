...
void SystemService::get_kernel_cmdline_serialno(void)
{
    ...
    */
    ...
    if(len1 < 3/* || len2 < 3*/)
    {
        ALOGD("xxxxxxxxxxxxxxxx length not right, len1 = %d, len2 = %d ,return! xxxx",len1, len2);
        return;
    }
    ...
    *(ptr2+len2) = 0;
    ...
    ret = property_set(CMCC_SERIALNO, serialno);
    ALOGD("xx (%s) ret:%d", serialno, ret);
    char value[256];
    ret = property_get(CMCC_SERIALNO, value, NULL);
    ALOGD("======================== get serialno! serialno = %s ret = %d\n", value, ret);
}
