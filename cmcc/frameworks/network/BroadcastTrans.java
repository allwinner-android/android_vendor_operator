...
public class BroadcastTrans {
    ...
    public BroadcastTrans(Context context) {
        mContext = context;
        ...
        if (EthernetManager.NETWORK_STATE_CHANGED_ACTION.equals(action)) {
            ...
            switch (event) {
                ...
                case EthernetManager.EVENT_PPPOE_DISCONNECT_SUCCESSED:
                    ...
                    break;
                case EthernetManager.EVENT_PPPOE_CONNECT_STARTED:
                    if (mode.equals(EthernetManager.ETHERNET_CONNECT_MODE_PPPOE)) {
                        notifyPppoeConnectStarted();
                    }
                    break;
                default:
                    break;
    ...
    private void sendStateBroadcast(NetworkInfo mNetworkInfo,
            LinkProperties mLinkProperties, int event) {
    ...
    private void notifyPppoeDisconnected() {
        ...
        try {
            ...
            intent.putExtra(PppoeManager.EXTRA_PPPOE_STATE,
                    PppoeManager.EVENT_CONNECT_DISCONNECTED);
            mContext.sendBroadcastAsUser(intent, UserHandle.ALL);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void notifyPppoeConnected() {
        ...
        try {
            Intent intent = new Intent();
            intent.setAction(PppoeManager.PPPOE_STATE_CHANGED_ACTION);
            intent.putExtra(PppoeManager.EXTRA_PPPOE_STATE, PppoeManager.EVENT_CONNECT_SUCCESSED);
            mContext.sendBroadcastAsUser(intent, UserHandle.ALL);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void notifyPppoeConnectFailed() {
        Log.d(TAG, "notifyConnectFailed");
        try {
            Intent intent = new Intent();
            intent.setAction(PppoeManager.PPPOE_STATE_CHANGED_ACTION);
            intent.putExtra(PppoeManager.EXTRA_PPPOE_STATE, PppoeManager.EVENT_CONNECT_FAILED);
            mContext.sendBroadcastAsUser(intent, UserHandle.ALL);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void notifyPppoeConnectStarted() {
        Log.d(TAG, "notifyPppoeConnectStarted");
        try {
            Intent intent = new Intent();
            intent.setAction(PppoeManager.PPPOE_STATE_CHANGED_ACTION);
            intent.putExtra(PppoeManager.EXTRA_PPPOE_STATE, PppoeManager.EVENT_CONNECT_STARTED);
            mContext.sendBroadcastAsUser(intent, UserHandle.ALL);
        } catch (Exception e) {
            e.printStackTrace();
        }
